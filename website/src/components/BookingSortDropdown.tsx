...
import { SortAsc, SortDesc, ArrowUpDown, Check, ChevronDown } from 'lucide-react';
...
const BookingSortDropdown: React.FC<BookingSortProps> = ({
  options,
  currentSort,
  onSort
}) => {
  ...
  const getCurrentSortLabel = () => {
    ...
    const option = options.find(opt => opt.id === currentSort.field);
    if (!option) return 'Sort';
    return `${option.label} ${currentSort.direction === 'asc' ? '(A-Z)' : '(Z-A)'}`;
  };
      ...
      >
        {currentSort ? (
          currentSort.direction === 'asc' ? (
            <SortAsc size={14} className="mr-1.5 text-sky-600" />
          ) : (
            <SortDesc size={14} className="mr-1.5 text-sky-600" />
          )
        ) : (
          <ArrowUpDown size={14} className="mr-1.5 text-gray-500" />
        )}
        <span className="max-w-[100px] truncate">{getCurrentSortLabel()}</span>
...
export default BookingSortDropdown;